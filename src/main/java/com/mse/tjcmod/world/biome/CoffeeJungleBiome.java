...
package com.mse.tjcmod.world.biome;
...
import com.mse.tjcmod.entity.JoseeeesEntity;
import com.mse.tjcmod.block.CoffeeLogBlock;
import com.mse.tjcmod.block.CoffeeLeavesBlock;
import com.mse.tjcmod.MseTjcmodModElements;

import com.google.common.collect.Lists;

@MseTjcmodModElements.ModElement.Tag
public class CoffeeJungleBiome extends MseTjcmodModElements.ModElement {
	@ObjectHolder("mse_tjcmod:coffee_jungle")
	public static final CustomBiome biome = null;
	public CoffeeJungleBiome(MseTjcmodModElements instance) {
		super(instance, 36);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.7999999999999999f).depth(0.1f).scale(0.2f).temperature(1.1f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.JUNGLE).waterColor(-14201179).waterFogColor(-14201179).parent("jungle")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.GRASS_BLOCK.getDefaultState(),
							Blocks.DIRT.getDefaultState(), Blocks.DIRT.getDefaultState())));
			setRegistryName("coffee_jungle");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(CoffeeLogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(CoffeeLeavesBlock.block.getDefaultState()))).baseHeight(7)
			...
			addFeature(GenerationStage.Decoration.UNDERGROUND_ORES,
					Feature.DISK
							.withConfiguration(new SphereReplaceConfig(Blocks.GRAVEL.getDefaultState(), 6, 2,
									Lists.newArrayList(Blocks.GRASS_BLOCK.getDefaultState(), Blocks.DIRT.getDefaultState())))
							.withPlacement(Placement.COUNT_TOP_SOLID.configure(new FrequencyConfig(1))));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(JoseeeesEntity.entity, 15, 1, 5));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == CoffeeLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, CoffeeLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, CoffeeLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == CoffeeLogBlock.block.getDefaultState().getBlock()
					|| blockType == CoffeeLeavesBlock.block.getDefaultState().getBlock()
